      ...
      <div className="w-full max-w-sm text-center">
        {status === 'verifying' && (
          <>
            <div className="mx-auto flex h-16 w-16 items-center justify-center">
              <svg className="h-8 w-8 text-text-secondary animate-spin" fill="none" viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
              </svg>
            </div>
            <h1 className="mt-4 text-2xl font-bold text-text-primary">Verifying your email...</h1>
        ...
        {status === 'success' && (
          <>
            ...
            <h1 className="mt-4 text-2xl font-bold text-text-primary">Email verified!</h1>
            <p className="mt-2 text-sm text-text-secondary">Redirecting to dashboard...</p>
          </>
        )}
        ...
        {status === 'error' && (
          <>
            ...
            <h1 className="mt-4 text-2xl font-bold text-text-primary">Verification failed</h1>
            <p className="mt-2 text-sm text-red-400">{error}</p>

            <div className="mt-6 space-y-3">
              <p className="text-sm text-text-secondary">Need a new verification link?</p>
              <div className="flex gap-2">
                <input
                  type="email"
                  value={resendEmail}
                  onChange={(e) => setResendEmail(e.target.value)}
                  className="flex-1 rounded-[2px] border border-border-hover bg-surface px-3 py-2 text-sm text-text-primary placeholder-text-tertiary focus:border-brand-accent focus:outline-none focus:ring-1 focus:ring-brand-accent"
