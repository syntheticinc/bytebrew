...
export default function Modal({ open, onClose, title, children, footer, className = 'max-w-lg' }: ModalProps) {
  ...
  return (
    <dialog
      ref={dialogRef}
      onClose={onClose}
      className={`rounded-card border border-brand-shade3/15 bg-brand-dark-alt text-brand-light p-0 backdrop:bg-black/60 backdrop:backdrop-blur-sm w-full shadow-xl animate-modal-in ${className}`}
    >
      <div className="p-6">
