...
// TaskLister retrieves tasks for a given session (legacy interface).
...
// Legacy version — returns plain string via GetReminder.
...
func (p *TaskReminderProvider) GetReminder(ctx context.Context, sessionID string) string {
	...
	for _, t := range tasks {
		...
		if t.ParentTaskID != nil {
			sb.WriteString(fmt.Sprintf(" (sub-task of %s)", t.ParentTaskID.String()))
		}
		sb.WriteString("\n")
	}
