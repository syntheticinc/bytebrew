...
func (r *GORMCapabilityRepository) ListByAgent(ctx context.Context, agentName string) ([]CapabilityRecord, error) {
	...
	for _, c := range caps {
		...
		if err != nil {
			return nil, fmt.Errorf("convert capability %d: %w", c.ID, err)
		}
...
func (r *GORMCapabilityRepository) GetByID(ctx context.Context, id uint) (*CapabilityRecord, error) {
	var cap models.CapabilityModel
	if err := r.db.WithContext(ctx).Preload("Agent").First(&cap, id).Error; err != nil {
...
func (r *GORMCapabilityRepository) Update(ctx context.Context, id uint, record *CapabilityRecord) error {
	configJSON, err := json.Marshal(record.Config)
	if err != nil {
		return fmt.Errorf("marshal capability config: %w", err)
	}
	...
	if result.Error != nil {
		return fmt.Errorf("update capability %d: %w", id, result.Error)
	}
...
func (r *GORMCapabilityRepository) Delete(ctx context.Context, id uint) error {
	result := r.db.WithContext(ctx).Delete(&models.CapabilityModel{}, id)
	if result.Error != nil {
		return fmt.Errorf("delete capability %d: %w", id, result.Error)
	}
	if result.RowsAffected == 0 {
		return gorm.ErrRecordNotFound
	}
...
func (r *GORMCapabilityRepository) resolveAgentID(ctx context.Context, agentName string) (uint, error) {
