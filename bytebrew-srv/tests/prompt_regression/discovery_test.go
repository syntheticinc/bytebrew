...
func TestDiscoveryBeforeTask(t *testing.T) {
	...
	fixture, err := LoadFixture("discovery_before_task_v1")
	if err != nil {
	...
	AssertFirstToolIsDiscoveryOrClarification(t, result)
}

func TestResearcherDelegation(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 120*time.Second)
	defer cancel()
	...
	fixture, err := LoadFixture("researcher_delegation_v1")
	if err != nil {
		t.Fatalf("LoadFixture: %v", err)
	}
	...
	// Supervisor should research first — either directly or via researcher spawn
