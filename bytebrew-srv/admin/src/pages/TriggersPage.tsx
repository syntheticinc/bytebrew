...
export default function TriggersPage() {
  ...
  const columns = [
    { key: 'title', header: 'Title' },
    {
      ...
      header: 'Type',
      render: (row: Trigger) => (
        <span className={`px-2 py-0.5 rounded text-xs font-medium ${row.type === 'cron' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'}`}>
          {row.type}
        </span>
      ),
    },
  ...
  if (loading) return <div className="text-gray-500">Loading triggers...</div>;
  ...
  return (
    <div>
      ...
      >
        <p className="text-sm text-gray-600">Delete this trigger? This action cannot be undone.</p>
      </Modal>
    </div>
  );
}
