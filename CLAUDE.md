...
7. ВЕРИФИКАЦИЯ PHASE (ОБЯЗАТЕЛЬНО после каждой phase)
   - Запустить code-reviewer агента ИЛИ самостоятельно пройти:
     a. Сверка КАЖДОЙ задачи из phase plan → реализована? файл существует?
     b. Сверка КАЖДОГО AC из phase plan → покрыт тестом или ручной проверкой?
     c. Матрица: задача/AC → PASS/FAIL → если FAIL → вернуться к п.1
   - go build OK, go test OK (новые пакеты)
   - git commit с пометкой "Phase N: [название] — complete"

8. ФИНАЛИЗАЦИЯ
   - Все AC пройдены
   - Все тесты проходят
   - → Перейти к Phase N+1 БЕЗ остановки
```

### Правила автономной работы

- **Не спрашивай пользователя** по ходу реализации — реализуй все phases подряд
...
- **Верификация phase** — после каждой phase обязательно перепроверить полноту (задачи + AC)
...
5. На checkpoint'ах — сообщи пользователю результат, НЕ останавливайся — продолжай следующую phase
