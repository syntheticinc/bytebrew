...
**Results:** 4 PASS, 2 PARTIAL, 0 FAIL, 0 SKIP
**Pass rate (runnable):** 4/6 (67%), 6/6 (100% including PARTIAL)
...
| TC-TESTFLOW-SESSION-01 | Session dropdown + switch | ✅ PASS | ✅ SCC-01 OK | Session ID appears in dropdown after chat. Messages visible. "View in Inspect" link works. Session persisted across page navigation (canvas → agents). |
| TC-TESTFLOW-SESSION-02 | Delete session | ⚠️ PARTIAL | ✅ SCC-01 OK | Delete button visible in code. Interactive test blocked by BUG-001 (canvas page snapshot issue). API endpoint verified: DELETE /sessions/{id} returns 401 without token. |
